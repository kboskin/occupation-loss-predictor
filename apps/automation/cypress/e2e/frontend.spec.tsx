describe('Desktop tests (1920x1080)', () => {
  ...
  it('Shows the correct number of days', () => {
    cy.visit('/')
    cy.get('div.text-5xl').first().should('exist')
    cy.wait(1200)
    cy.get('div.text-5xl').first().invoke('text').then((timerText) => {
  ...
  it('finds the content "AI-Based Russian Losses Forecast"', () => {
    cy.visit('/')
    cy.contains('AI-Based Russian Losses Forecast')
      .should('be.visible')
  })

  it('finds the content "Protecting the World"', () => {
    cy.visit('/')
    cy.contains('Protecting the World')
      .should('be.visible')
  })
